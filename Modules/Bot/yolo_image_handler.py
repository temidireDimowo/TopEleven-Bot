...
class YOLOImageHandler:
    """Handles image detection using a custom YOLO11 model."""
    ...
    def _load_model(self) -> None:
        ...
        try:
            ...
            if hasattr(self.model, 'names') and self.model.names:
                ...
                self.logger.warning("Could not retrieve class names from model")
                
        except Exception as e:
            self.logger.error(f"Failed to load YOLO11 model: {e}")
            raise
    
    def find_objects_on_screen(
        self,
        target_classes: List[str] = None, 
        confidence_threshold: float = None
    ) -> List[Dict]:
        """
        Find objects on screen using the YOLO11 model.
            ...
            if results and len(results) > 0:
                ...
                if result.boxes is not None and len(result.boxes) > 0:
                    ...
                    for i, (bbox, confidence, class_id) in enumerate(
                        ...
                        self.logger.debug(
                            f"Found {class_name} at ({center_x}, {center_y}) with confidence {confidence:.2f}"
                        )
    ...
    def find_best_match(
        ...
        self.logger.info(
            f"Best match: {best_match['class_name']} (confidence: {best_match['confidence']:.2f})"
        )
        return best_match
    
    def find_class_on_screen(
        self,
        class_name: str, 
        confidence_threshold: float = None
        ...
        Maintains compatibility with the existing image_handler interface.
        """
    ...
    def wait_for_class(
        ...
        Returns:
            True if class was found, otherwise False after timeout.
        """
        ...
        self.logger.warning(f"{class_name} not found after {timeout} seconds")
        return False
